...
inline bool box_compare(const shared_ptr<hittable> a, const shared_ptr<hittable> b, int axis)
{
    ...
    {
        cerr << "No bounding box in bvh node constructor.\n";
    }
...
bvh_node::bvh_node(const vector<shared_ptr<hittable>> &src_objects, size_t start, size_t end, double time0, double time1)
{
    ...
    bool (*comparator)(shared_ptr<hittable> a, shared_ptr<hittable> b) = (axis == 0) ? box_x_compare : (axis == 1) ? box_y_compare : box_z_compare;
